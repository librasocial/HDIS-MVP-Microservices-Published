...
channel.queue_bind(queue='hdis_consultation_assessment', exchange='hdis_facility')
channel.queue_bind(queue='hdis_consultation_assessment', exchange='hdis_doctor_administration')
channel.queue_bind(queue='hdis_consultation_assessment', exchange='hdis_patient_registration')
channel.queue_bind(queue='hdis_consultation_assessment', exchange='hdis_appointment_management')
channel.queue_bind(queue='hdis_consultation_assessment', exchange='hdis_visit_management')
...
def callback(ch, method, properties, body):
    ...
    if properties.headers['content_type'] == 'facility created':
        ...
        data = json.loads(body)
        print(data)
        serializer = outreachSerializers(data=data)
        serializer.is_valid(raise_exception=True)
        try:
            outreach.objects.get(encounterId=data['encounterId'])
